...
@synthesize listProcedure;
@synthesize listCountry;
@synthesize listCity;
@synthesize listMedicalCenter;
@synthesize listAllCountries;
@synthesize listPriceMedicalCenter;
@synthesize listAveragePriceMedicalCenter;
@synthesize listFeaturePriceMedicalCenter;

@synthesize redirectLink;
@synthesize pageInfo;
@synthesize pageFInfo;
@synthesize pageAInfo;

@synthesize placidCall;
@synthesize placidMail;
...
-(id)init {
    ...
    if (self = [super init]) {
        ...
        listProcedure = [[NSMutableArray alloc] init];
        listCountry = [[NSMutableArray alloc] init];
        listCity = [[NSMutableArray alloc] init];
        
        listMedicalCenter = [[NSMutableArray alloc] init];
        listPriceMedicalCenter = [[NSMutableArray alloc] init];
        listFeaturePriceMedicalCenter = [[NSMutableArray alloc] init];
        listAveragePriceMedicalCenter = [[NSMutableArray alloc] init];

        redirectLink = @"";

        placidCall = @"+1-303-500-3821";
        placidMail = @"info@placidway.com";
    }
...
+(NSString*)getNibNameFromName:(NSString*)name {
    ...
    if (name)
    {
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
        {
            NSLog(@"Screen height : %f",MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width));

            if (MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width) == 480.0 ||
                MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width) == 960.0)//3.5
            {
                [nibName appendString:@"~iphone"];
            }
            else if (MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width) == 568.0)//4
            {
                [nibName appendString:@"_iPhone5~iphone"];
            }
            else if (MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width) == 667.0)//4.7
            {
                [nibName appendString:@"_iPhone6~iphone"];
            }
            else if (MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width) == 736.0)//5.5
            {
        ...
        {
            if (MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width) == 1024.0)
            {
                [nibName appendString:@"~ipad"];
            }
            else if (MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width) == 2048.0)
            {
                [nibName appendString:@"_iPadR~ipad"];
            }
        }
    }
    NSLog(@"%@ : %f",nibName,MAX([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width));
    return nibName;
}
...
{
    UIAlertView *Alert=[[UIAlertView alloc] initWithTitle:@"Alert!" message:@"Sorry No Pricing Found." delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [Alert show];
}
-(void)showConnectionErrorAlert
{
    UIAlertView *Alert=[[UIAlertView alloc] initWithTitle:@"Connection Failed!" message:@"Please check your internet connection." delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [Alert show];
}
